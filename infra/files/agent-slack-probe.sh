#!/bin/bash
# agent-slack-probe.sh - host-side Slack liveness watchdog for a tenant.
#
# Two rules trigger a `docker restart $CONTAINER_NAME`:
#
...
#
# Deployed to /opt/<agent-name>/bin/ via infra/tasks/agent_new.py.
# Fired every 3 minutes by <agent-name>-slack-probe.timer.

set -euo pipefail

CONTAINER="${CONTAINER_NAME:-zeroclaw}"
STATE_DIR="/var/lib/${CONTAINER}-probe"
STATE_FILE="$STATE_DIR/consecutive_failures"
LOG_FILE="/var/log/${CONTAINER}-probe.log"
MAX_FAILURES=3
ZOMBIE_WINDOW=6h
